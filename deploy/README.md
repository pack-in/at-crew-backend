...
### `.env`의 `APP_IMAGE`는 배포가 갱신한다

배포(워크플로·스크립트 양쪽)는 띄운 이미지를 서버 `.env`의 `APP_IMAGE`에도 기록한다(`deploy/write-app-image.sh`).
그래서 서버에서 `docker-compose … up -d`로 컨테이너를 다시 만들어도 같은 이미지가 올라온다.

기록은 **이미지를 받아온 뒤**에 한다. 먼저 쓰면 `pull`이 실패했을 때 `.env`가 받을 수 없는 이미지를 가리킨
채 남아, 그 뒤 재생성이 아무것도 띄우지 못한다.

`.env`의 **소유자와 권한은 원본에서 승계한다**(`chown/chmod --reference`). SSM은 root로 도는데 새 파일을
만들어 옮기면 `.env`가 root 소유가 되고, `User=ec2-user`로 도는 백업 서비스가 `.env`를 읽지 못해 조용히
멈춘다 — 같은 계열의 사고가 2026-09-11에 있었다(`deploy/bootstrap.sh` 주석). 임시 파일은 `umask 077`로
만들어 시크릿이 한순간도 다른 계정에 읽히지 않게 한다.

`deploy/`에 `.env.next`가 남아 있으면 **쓰기가 중간에 끊긴 것**이다. 시크릿 사본이므로 내용을 확인한 뒤
지운다. 일부러 `.gitignore`에 넣지 않았다 — 드리프트 검사(`git ls-files --others`)에 걸려야 알 수 있다.

예전에는 셸 변수로만 넘겨서 `.env`가 옛 이미지를 가리킨 채 남았다. 그 상태로 서버에서 컨테이너를
재생성하면 **옛 이미지로 조용히 되돌아가고**, 그 버전이 이미 적용된 마이그레이션과 맞지 않으면 기동조차
못 한다 — 2026-09-23에 `artwork_images`를 지운 뒤(V47) 8월 이미지로 되돌아가 3분간 중단됐다.

롤백 단계도 같은 방식으로 `.env`를 갱신한다. 그러지 않으면 `.env`가 방금 실패한 이미지를 계속 가리킨다.
