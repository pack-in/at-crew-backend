...
`main`과 `dev`에 같은 보호 규칙이 걸려 있다.

| 항목 | 값 |
|---|---|
| 필수 상태 검사 | `build-and-test`, `validate-pr-title` — **실패·진행 중이면 병합 버튼이 막힌다** |
| 최신화 강제(strict) | 끔 — base가 앞서 있어도 재실행을 요구하지 않는다 |
| force push | 금지 |
| 브랜치 삭제 | 금지 |
| 대화 해결 필수 | 켬 |
| 관리자 강제 | 끔 — 저장소 소유자는 비상시 우회할 수 있다 |

> `dev`는 2026-09-02까지 보호 규칙이 **아예 없었다.** CI는 `dev` 대상 PR에서도 돌았지만 강제되지
> 않아 **검사가 진행 중이거나 실패한 상태로도 병합이 됐다.** `main`은 그때도 막혀 있어 프로덕션에
> 잘못된 코드가 갈 위험은 없었으나, `dev`가 깨진 채로 쌓일 수 있는 구멍이었다. 브랜치 삭제 금지는
> 과거 `origin/dev`가 삭제됐던 사고의 재발 방지를 겸한다.

**다만 "PR의 base를 제한"하는 규칙은 GitHub에 없다.** 작업 브랜치에서 `main`으로 바로 PR을 여는
것은 여전히 막을 방법이 없으므로 위 브랜치 흐름은 **규율로 지킨다.**
...
### `dev`로 보낸 PR은 `Closes`로 이슈가 닫히지 않는다

GitHub은 **기본 브랜치(`main`)에 병합될 때만** `Closes #N`을 처리한다. 이 저장소는 작업
브랜치가 전부 `dev`로 가므로 **`Closes`가 사실상 한 번도 자동으로 동작하지 않는다.**

그렇다고 본문에서 빼지는 않는다 — `Closes`는 PR과 이슈를 양방향으로 연결해 주고, 나중에
`dev` → `main`이 열릴 때 실제로 닫히기도 한다.

**병합한 사람이 이슈를 직접 닫는다.** 닫을 때 아래 둘을 코멘트로 남긴다.

- 어느 PR·커밋으로 처리됐는지
- **언제 실제로 적용되는지** — 코드가 `dev`에 들어간 것과 프로덕션에서 동작하는 것은 다르다.
  배포는 `dev` → `main`에서 일어나고, 호스트 설정(`deploy/bootstrap.sh`)은 배포와도 무관하게
  사람이 서버에서 실행해야 반영된다

아직 적용되지 않았는데 이슈가 닫혀 있으면 "끝난 일"로 읽혀서 아무도 다시 보지 않는다.
