...
@Schema(description = """
        ...
        아래 예시는 "선택 프로필 항목을 하나도 채우지 않은 갓 가입한 계정" 기준이다 — 실제로는 필드 설명에
        적힌 대로 값이 있으면 그 필드가 추가로 나타난다.""")
...
public record MemberInfo(
        ...
        @ArraySchema(arraySchema = @Schema(description = """
                ...
                중복 제거 후 위 나열 순서로 정렬돼 내려가며, 비어 있으면 응답에서 생략된다(가입 직후 기본 상태).
                값이 있을 때의 예시: ["ILLUSTRATION","WEBTOON"]"""))
        List<ActivityField> activityFields,

        // === 활동 경력 [피그마: 활동 경력 칩 단일 선택] ===
        @Schema(description = """
                활동 경력 연차. NEWCOMER — 신입, ONE_TO_TWO — 1-2년차, THREE_TO_FOUR — 3-4년차,
                FIVE_TO_NINE — 5-9년차, TEN_PLUS — 10년차 이상. 미설정이면 응답에서 생략된다(가입 직후 기본 상태)""",
                nullable = true)
        ...
        @ArraySchema(arraySchema = @Schema(description = """
                ...
                중복 제거 후 위 나열 순서로 정렬돼 내려가며, 비어 있으면 응답에서 생략된다(가입 직후 기본 상태).
                값이 있을 때의 예시: ["SHORT_TERM","DIVISION"]"""))
        ...
        @Schema(description = "연락처 (전화번호 또는 이메일). 미설정이면 응답에서 생략된다(가입 직후 기본 상태). "
                + "값이 있을 때의 예시: 010-1234-5678", nullable = true)
        String contact,

        @Schema(description = "SNS 링크. 미설정이면 응답에서 생략된다(가입 직후 기본 상태). "
                + "값이 있을 때의 예시: https://x.com/testcreator", nullable = true)
        String sns,

        @Schema(description = "사용 가능한 툴. 미설정이면 응답에서 생략된다(가입 직후 기본 상태). "
                + "값이 있을 때의 예시: Clip Studio Paint, Photoshop", nullable = true)
        String tools,
        ...
        @Schema(description = "탈퇴 처리 시각 (UTC ISO-8601). **활성 회원이면 항상 응답에서 생략된다** — "
                + "이 필드가 존재한다는 것 자체가 탈퇴 회원이라는 뜻이며, 정상 로그인·조회 응답에는 결코 나타나지 않는다",
                nullable = true)
        Instant deletedAt,

        @Schema(description = "마지막 로그인 시각 (UTC ISO-8601). 이번이 첫 로그인(가입 직후)이면 응답에서 생략된다. "
                + "값이 있을 때의 예시: 2026-08-12T01:06:18.746484Z", nullable = true)
        Instant lastLoginAt,
