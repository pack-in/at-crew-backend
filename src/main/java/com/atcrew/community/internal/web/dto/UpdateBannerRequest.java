...
public record UpdateBannerRequest(
        @Schema(description = "배너 이미지 URL — null이면 기존 값 유지",
                example = "https://cdn.atcrew.com/banners/spring-event-v2.png", nullable = true)
        String imageUrl,

        @Schema(description = "배너 클릭 시 이동할 링크 URL — null이면 기존 값 유지",
                example = "https://atcrew.com/events/spring-v2", nullable = true)
        String linkUrl,

        @Schema(description = "노출 순서 — null이면 기존 값 유지. 값을 바꾸면 기존 순번과 새 순번 사이의 "
