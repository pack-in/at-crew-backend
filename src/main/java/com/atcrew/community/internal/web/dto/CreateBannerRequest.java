...
public record CreateBannerRequest(
        ...
        @Schema(description = "배너 이미지 URL (최대 500자)", example = "https://cdn.atcrew.com/banners/spring-event.png",
                requiredMode = Schema.RequiredMode.REQUIRED)
        @NotBlank @Size(max = 500) String imageUrl,

        @Schema(description = "배너 클릭 시 이동할 링크 URL (최대 500자)", example = "https://atcrew.com/events/spring",
                requiredMode = Schema.RequiredMode.REQUIRED)
        @NotBlank @Size(max = 500) String linkUrl,

        @Schema(description = "노출 순서(0 이상). 미지정 시 마지막 순번 다음 값이 자동 부여된다(배너가 없으면 0). "
