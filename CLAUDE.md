...
**라이트는 종료하지 않는다(2026-09-10 결정).** 앳크루와 별개의 서비스로 계속 운영한다. 따라서
라이트 → 앳크루 데이터 마이그레이션도, 그것을 위한 **데이터 모델 하위 호환 제약도 더 이상 없다.**
라이트의 도메인 모델은 **참고 자료**로 쓰되 설계를 거기에 맞출 이유는 없다 — 애초에 그 기술 부채를
벗어나려고 다시 쓴 프로젝트다.
