...
- **5번(결제/구독) PG 확정: Polar** — 구독형(스타터 무료/프로 월·연)+단건 게시 상품(팀원모집글 업로드권한·끌어올리기) 2종. 환불·구독취소·결제수단변경은 전부 Polar 담당, 앳크루는 노출·진입·웹훅 반영만. **(2026-08-10 정정: 이번 출시 마일스톤은 Stripe로 진행 — `billing-module-design.md` §7)**
...
| 4 | 검색 모듈 | **완료**, main 머지(Elasticsearch 기반). `RecruitSearchPort` 스텁 교체 완료(2026-08-01, PR #41) — recruit 3종 검색은 search 위임이 아닌 DB LIKE 기반으로 구현, ES 색인 이관은 별도 스코프. **2026-08-10 출시 마일스톤에서 태그 매칭 버그 수정(`tags` 필드가 ES Keyword라 사실상 매칭 안 되던 것) 착수** — `settings-i18n-design.md`·상위 실행 계획 참고 |
| 5 | 결제/구독(요금제) | PG를 Polar에서 **Stripe로 정정**(2026-08-10), 구독형만 구현 착수 — `docs/design/billing-module-design.md` |
| 6 | 설정 나머지(로그아웃/비밀번호 변경 등) | **2026-08-10 설계 완료, 구현 착수** — `docs/design/settings-i18n-design.md` |
...
**PG 확정(2026-07-31, 기획서 정본): Polar** — 결제·구독 관리·환불·결제수단 변경은 전부 Polar에서 처리하고, 앳크루 백엔드는 (1) 상품 노출 (2) Polar Checkout 진입 (3) 웹훅 수신 후 권한 반영만 담당한다. 자체 결제 UI·PG 직접 연동 없음.

> **2026-08-10 정정 — 이번 출시 마일스톤은 Stripe로 진행한다.** 아래 상품 구조·정책은 기획서 원문(Polar 기준)을 그대로 보존하고, 실제 구현은 Stripe Checkout + Customer Portal + Webhook으로 대체한다. Stripe는 Polar와 달리 Merchant of Record가 아니라 VAT·인보이스 처리 주체가 앳크루로 이동한다 — 근거·매핑·미확정 사항은 `docs/design/billing-module-design.md` §7 참고. 이번 마일스톤은 **구독형 상품만** 구현하고 단건 게시 상품(팀원모집글 업로드권한·끌어올리기)은 범위 밖이다(recruit 자체가 이번 마일스톤에 없음).

**상품 구조 (기획서 요금제-R02~R06 기준):**
- 구독형: 스타터(무료, 전 가입자 기본)/프로 월간(7,500원, 정가 15,000원)/프로 연간(75,000원, 정가 150,000원). 스타터↔프로 전환·월간↔연간 변경.
- 단건 게시 상품: 팀원 모집글 업로드 권한(50,000원, 끌어올리기 1회 포함)/끌어올리기(10,000원). 구매 시 보유 개수 +1, 게시 성공 시에만 차감(실패 시 미차감), 만료 없음. (이번 마일스톤 범위 밖)
- 정기 결제 실패: (Polar→Stripe) 웹훅 기준 판별, 유예 없이 즉시 스타터 전환+이메일 알림, 재결제 성공 시 웹훅 기준 복원.
- 탈퇴 시 (Polar Delete Customer API → Stripe Subscription cancel + Customer delete)로 활성 구독 즉시 취소(잔여 기간 환불 여부는 Stripe 정책 확인 필요 — 미확정).

**스코프:**
- 요금제 페이지(비로그인, 스타터/프로/단건 카드), 설정 > 요금제 및 결제 탭(로그인, 프로+단건 카드만)
- (Polar → Stripe) Checkout 연동, 웹훅 핸들러(결제 성공/실패/구독 변경/취소)
- 플랜 다운그레이드 시 기존 산출물 유지 정책(REQ-020 — 공유 링크·포트폴리오는 유지, 신규 생성만 제한)
- 정기 결제 실패 이메일 알림 — 이메일 발송 인프라는 이번 마일스톤에서 Resend로 함께 구축(`docs/design/settings-i18n-design.md` §1), 비밀번호 재설정 메일과 발송 경로 공용
