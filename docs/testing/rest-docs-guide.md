...
**(2026-08-18 구현, 2026-08-19 Cloudflare Pages로 전환)** 프론트엔드에 API 문서를 지속적으로 최신
상태로 전달하기 위해 아래 파이프라인을 구축했다: `OpenApiExportTest`(`src/test/java/com/atcrew/support/`)가
`RestDocsIntegrationSupport`의 전체 Spring 컨텍스트에서 `GET /v3/api-docs`를 호출해
`build/openapi/openapi.json`으로 저장하고, `.github/workflows/docs.yml`이 main push마다 이 테스트를
실행해 `docs/api-site/index.html`(정적 Swagger UI, CDN 번들 사용)과 함께 **Cloudflare Pages**
(`https://at-crew-api-docs.pages.dev/`)로 배포한다. GitHub Pages가 아닌 이유는 `pack-in` 조직이
GitHub Free 플랜이라 private 저장소의 GitHub Pages를 쓸 수 없기 때문(Pro/Team 이상 필요) — Cloudflare는
이미 DNS·R2로 쓰고 있어 새 벤더 계정 없이 바로 붙일 수 있었다. prod는 보안상 springdoc이 꺼져 있어
