...
> **정정(2026-09-10).** 위 3번의 전제가 사라졌다. 라이트를 종료하지 않고 **앳크루와 별개의 서비스로
> 계속 운영**하기로 해서, 라이트 → 앳크루 데이터 이관도 그것을 위한 하위 호환 제약도 없다(`CLAUDE.md`
> 프로젝트 개요). 라이트 데이터의 provider를 추론해 채우는 백필은 하지 않는다. 1·2번(복합 조회 API와
> 복합 unique index)은 앳크루 자체의 설계이므로 그대로 유효하다.
