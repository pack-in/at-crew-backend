...
| 상품 키 | 유형 | 청구가 | 정가 | 상태 | 설명 |
|---------|------|--------|------|------|------|
| `PRO_MONTHLY` | 구독(월) | $8.00 | — | 판매중 | MVP 단일가(PH-08, 2026-08-18), 정가 취소선 없음 |
| `PRO_YEARLY` | 구독(연) | $80.00 | $96.00 | 판매중 | 2개월 무료 — 재개(PH-09, 2026-08-23) |
| `TEAM_POSTING` | 단건 | $39.99 | — | 판매중단 | 팀원 모집글 업로드권한(+끌어올리기 1회). 판매만 중단, 기존 보유자 게이팅은 유지(PH-08) |
| `BOOST` | 단건 | $7.99 | — | 판매중단 | 끌어올리기 1회(48시간 상단 고정). PH-08 |
| `JOB_POSTING` | 단건 | $99.99 | — | 판매중단 | 구인글 업로드권한(기업 전용). PH-08 |

정가는 표시 전용이라 Stripe에 만들지 않고 `billing.products.*.list-amount` 설정으로만 관리한다.
단건 권한은 만료되지 않으며 **게시가 성공했을 때만** 차감한다(요금제-R06). 판매중단 상품은
`billing.products.*.enabled=false`로 카탈로그·Checkout 양쪽에서 막힌다(`plans/260813-stripe-billing/PLAN-HUMAN.md` PH-08).
`PRO_MONTHLY`·`PRO_YEARLY`는 Stripe Product 하나(`Creator Pro`)에 Price 2개로 등록돼 있다 —
