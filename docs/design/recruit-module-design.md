...
게시글을 지우면 `status = DELETED`로 표시만 하고(soft delete) 이미지는 그대로 남는다. recruit에는 작품처럼
영구 삭제 경로가 없어 R2 파일이 무기한 쌓였다.

`RecruitTrashImageCleanupScheduler`가 1시간마다 휴지통 보관 기간(기본 1년, `recruit.trash.retention`)이
지난 게시글의 이미지를 정리한다. 대상은 media 자산(원본·변형본)과, 자식 행이 없던 시절의 레거시
컬럼(`thumbnail_image`, `reference_images`)이 가리키는 key다. 정리한 뒤 그 컬럼을 비운다 — 지운 파일의 key를
남겨두면 배치가 매번 같은 파일을 다시 지우려 한다.

**게시글 행과 지원 내역 파기는 기본으로 꺼져 있다**(`recruit.trash.purge-enabled=false`). 켜면 지원 내역을
먼저 지우고(게시글을 삭제 연쇄 없이 참조하므로 순서가 중요하다) 게시글 행을 지운 뒤 변경 이벤트를 발행해
검색 색인에서도 걷어낸다.

끄고 시작하는 이유는 지원 내역이 지원자의 개인정보이고, 보관 기간은 개인정보처리방침에 적어 동의를 받아야
하는 값이기 때문이다. 방침이 정해지면 `recruit.trash.retention`을 그 기간에 맞추고 이 설정을 켠다. 참고로
법·업계 관행은 "혹시 몰라 계속 보관"이 아니라 "정해진 기간 뒤 파기"이고, 5년 보관 의무는 결제·계약 기록에
붙지 지원 내역에는 붙지 않는다.

게시글마다 별도 트랜잭션으로 처리해 하나가 실패해도 배치 전체가 멈추지 않는다. R2 삭제가 실패하면 key를
고아 큐로 넘겨 정리 배치가 다시 시도한다.
...
## 구직 정보의 정본은 작가 프로필이다 (2026-08-26)

기획서 마이페이지_작가-R24에 따라 구직 정보(투입 가능 시기·희망 담당 업무·희망 장르·희망 채용 형태·
희망 근무 방식·선호 피드백 방식·희망 MG·희망 연봉)의 정본은 **작가 프로필**이며, 구직글은 작성 시점의
프로필 값을 복사해 온다. 프로필 쪽 구현은 V30에서 끝났고 `MemberProfileInfo`로 노출된다.

**서버 자동 복사는 아직 넣지 않았다.** 두 모듈의 값 집합이 달라 그대로 복사하면 값이 조용히 사라지거나
뒤바뀌기 때문이다.
...
당장은 클라이언트가 `MemberProfileInfo`를 읽어 구직글 작성 폼을 미리 채운다 — 손실이 없고 실제 UI 동작과
일치한다. 서버 복사를 도입하려면 위 값 집합 통일이 선행되어야 하며, 그 시점에 recruit 쪽 enum 교체 +
데이터 마이그레이션 + REST Docs 갱신이 함께 필요하다.

구직글은 MVP 범위가 아니라 이 결정이 당장 배포를 막지 않는다.
