...
페이지네이션 방식은 **화면이 무엇을 요구하는지로 갈린다.** 예전에는 "모든 목록은 커서, OFFSET 금지"였는데, 그 규칙은 artwork의 무한 스크롤 화면을 전제로 쓰인 것이라 번호 페이지네이션 화면에는 근거가 그대로 적용되지 않는다(2026-09-22 변경).

| 화면 | 방식 | 응답 봉투 |
|------|------|-----------|
| 무한 스크롤·"더 보기" 목록 | 커서(keyset) | `CursorPage<T>` |
| 번호 페이지네이션 목록 | 오프셋(`page`·`size`) | `CommunityPage<T>` |

커서 목록의 커서 형식·`hasNext` 판정은 artwork-module-summary.md §5와 동일하다. 오프셋을 쓸 때는 두 가지를 반드시 지킨다.

- **정렬은 (정렬 키, id) 2단으로 고정한다.** 정렬 키가 같은 행들의 순서를 DB가 보장하지 않아 페이지 경계에서 누락·중복이 생긴다(이슈 #78과 같은 이유).
- **최대 페이지 상한을 둔다.** OFFSET은 건너뛸 행을 실제로 읽으므로 깊은 페이지의 비용이 선형으로 늘어난다. 상한을 넘는 요청은 400으로 거부한다.
- **`size=0`은 조회 없이 빈 목록을 돌려준다.** 기존 동작(2026-08-28 prod 이슈)이고, `PageRequest`가 0을 거부하므로 컨트롤러에서 끊는다.

커뮤니티 4개 탭(`/artworks`·`/authors`·`/job-postings`·`/team-recruits`)은 Figma(`UI개편_커뮤니티`)가 하단에 번호 페이지네이션(`< 1 2 3 4 5 >`)을 쓰므로 **오프셋 방식**이다. 커서로는 임의 페이지 이동을 구현할 수 없다. 응답은 커뮤니티 전용 봉투 `CommunityPage<T>`이며 `totalCount`(전체 개수)와 페이지 정보를 담는다. 공용 `CursorPage`는 무한 스크롤 목록용으로 그대로 둔다.

`totalCount`는 목록과 같은 필터·뷰어 조건(언어 세그먼트·성인 콘텐츠 표시 설정)으로 센 값이고, 페이지 번호를 그리려면 매 요청 필요하므로 모든 페이지 응답에 담는다. 목록 조회와 개수는 **한 번의 호출로 함께** 얻는다(`common.response.OffsetPage`) — 목록과 개수를 서비스 메서드 둘로 나누면 조회 계층이 이미 센 개수를 버리고 같은 COUNT를 한 번 더 실행한다.

전환 작업은 `plans/260922-community-pagination/`에서 진행한다.
