...
- `SnapshotRetainedMediaKeyProvider`(portfolio) — 스냅샷마다 참조 key를 색인 테이블 `portfolio_snapshot_media_keys(snapshot_id, media_key)`(V41)에 두고, 후보 key로 바로 조회한다(`findActiveReferencedKeys`). 색인은 스냅샷 생성 시점에 카드 썸네일 2종·상세 이미지 4종·자료 첨부로 채우고(`ArtworkSnapshotPayload.mediaKeys()`), 스냅샷이 지워지면 FK CASCADE로 함께 지워진다. **포트폴리오 행이 남아있는 스냅샷만** 대상이라 포트폴리오를 지우면 다음 정리 배치에서 자연히 회수된다.
