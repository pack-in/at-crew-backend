...
| R3 | 복제 시 PRIVATE 제외 vs 생성 시 PRIVATE 포함 | **2026-08-12 PM 확정으로 최종 종결** — 완전 비공개의 정의는 "피드 OFF + 어느 포트폴리오에도 미포함"이며, PRIVATE 작품도 포트폴리오에 담을 수 있다(포트폴리오 편입 자체가 "한정 공개"로의 전환이라는 설계 의도). 즉 **현재 구현(`resolveOwnedArtworks`가 생성/수정 시 PRIVATE 포함 허용)이 맞다** — Figma 작품 선택 화면의 "비공개 작품은 포함할 수 없어요" 캡션이 구버전 문구였다. 코드 변경 없음, `docs/design/figma.md` "검증 중 확인된 이슈"도 종결로 갱신 |
