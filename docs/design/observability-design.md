...
- **수동 작업 중 억제** — 사람이 인프라를 건드릴 때는 `deploy/observability/silence.sh`로 직접 건다. CI 경로만 있던 동안에는 `GRAFANA_API_KEY`가 GitHub Secrets에만 있어 운영자가 알람을 끌 수단이 없었다(2026-09-11 NAT 탄력적 IP 재할당 때 못 걸고 진행). 이 스크립트는 CI와 같은 API를 쓰고, 자격증명은 레포 밖 `~/.config/at-crew/observability.env`에서 읽는다 — 앱 서버 `.env`에 두지 않는 이유는 서버가 침해됐을 때 알람을 끌 권한까지 함께 넘어가면 안 되기 때문이다.
...
- EC2 #1에서 일 1회(systemd 타이머, 18:00 UTC) `mariadb-dump` → gzip → **age 암호화** → R2 업로드. 보관 30일, 그 이상은 삭제.
- 백업 버킷은 이미지용 버킷과 분리하고 전용 키를 쓴다.
- 성공 시 타임스탬프를 Alloy의 textfile 컬렉터가 읽는 파일에 기록 → `atcrew_backup_last_success_timestamp` 메트릭.
- 26시간 이상 갱신이 없으면 P1.

**암호화(2026-09-10 추가).** 덤프는 age 공개키로 암호화한다. 서버에는 공개키(`AGE_RECIPIENT`)만 두고
복호화 개인키는 두지 않는다 — R2 토큰 유출, Cloudflare 계정 침해, **EC2 #1 침해** 셋 다 덤프를 읽는
데까지 이어지지 않는다. 대칭키나 SSE-C는 키가 `.env`에 있어야 해서 세 번째를 못 막는다.
앱·DB가 한 인스턴스에 모여 있는 구성이라 그 시나리오가 가장 중요하다.

개인키 분실이 유출보다 발생 확률이 높다. 보관 위치는 런북에 적고 사본을 두 곳에 둔다.
...
**근거.** R2에는 오브젝트 버저닝이 없다(`PutBucketVersioning` 미구현). 그런데 앱은 작품 삭제 시
R2 객체를 실제로 지운다 — 잘못된 key가 넘어가면 되돌릴 수단이 전혀 없다. 같은 Cloudflare 계정
안의 버킷이라 계정 침해는 못 막지만, 실제로 대비할 값어치가 있는 건 앱 버그·운영 실수 쪽이다.

P1이 아니라 P2인 이유: 중단돼도 이미 복사된 사본은 남아 있고, 위험한 것은 그 사이 새로 올라온
이미지뿐이다.

### 10.3. 검증

복구 절차는 런북에 기재한다. **복원 리허설을 최소 1회 수행하고 소요 시간을 런북에 적는다** — 해본 적 없는 백업은 백업이 아니다.
리허설은 복호화까지 확인해야 한다. 암호화만 되고 복호화가 안 되는 상태는 알람에 정상으로 잡힌다.
