...
> **후속(2026-09-10).** 이 절이 "제약이 실질적으로 비어 있다"고 정리한 뒤, 제약 자체가 아예
> 없어졌다. 라이트를 종료하지 않고 앳크루와 별개의 서비스로 계속 운영하기로 했다 —
> 라이트 → 앳크루 이관 계획이 사라졌으므로 아래 §7-3의 "라이트 이관은 별개 문제로 남는다"도
> 더 이상 예정된 작업이 아니다(`CLAUDE.md` 프로젝트 개요).
...
| `activeRegions`, `teamExperiences` (List\<enum\>) | 자식 테이블 `member_active_regions`, `member_team_experiences` — 동일 패턴 (※ `member_active_regions`는 이후 V26에서 제거됨 — 활동 지역이 단일 선택으로 확정되어 `members.active_region` 컬럼으로 전환) | 현재는 검색에 안 쓰이지만 성격상(지역 필터) 쓰일 가능성이 높고, enum 리스트 3종의 매핑 패턴을 하나로 통일하는 것이 유지보수에 유리. JSON으로 갈 실익 없음 |
...
#### 이미 배포된 마이그레이션 파일은 고치지 않는다

주석 한 줄도 고치면 안 된다. Flyway는 적용 시점의 체크섬을 기록해 두고 기동할 때마다 대조하므로, 내용이
달라지면 **운영에서 기동이 거부된다**(`Migration checksum mismatch`). 고칠 내용이 있으면 새 마이그레이션
파일을 추가한다.

이 실수는 테스트로 드러나지 않는다. 테스트와 CI는 매번 빈 DB에서 전체 마이그레이션을 새로 적용해 체크섬이
항상 일치하기 때문이다. 이미 적용된 DB에서만 나타난다.

**2026-09-23 사고**: PR #219가 V41의 주석 한 줄을 고쳤고(내용은 "첨부 key를 V46에서 채운다"는 설명 보강),
CI는 통과했지만 배포 후 운영 앱이 기동을 거부해 약 6분간 중단됐다. 배포에 마이그레이션이 포함돼 자동 롤백도
돌지 않았다. 복구는 적용 기록의 체크섬을 현재 파일 값으로 맞춘 뒤 재기동했다 — SQL이 동일함을 배포된 커밋과
직접 비교해 확인한 뒤에 했다. 재발 방지로 CI에 `guard-applied-migrations` 잡을 두어, main에 이미 있는
마이그레이션 파일이 PR에서 수정·삭제·이름변경되면 실패시킨다.
...
## 13. P4(artwork 절반)에서 발견한 함정 (다음 Phase 참고용)

- **자식 엔티티에 순서 전용 UNIQUE(부모FK, ordinal) 제약이 있으면 "컬렉션 전체 교체"가 그 자체로 깨진다**: `artwork_images`/`artwork_materials`처럼 `UNIQUE(artwork_id, ordinal)`이 걸린 자식 테이블에서, 기존 원소를 지우고 같은 ordinal 범위(0..N-1)로 새 원소를 채워 넣는 "전체 교체"를 한 트랜잭션의 한 flush에서 처리하면 실패한다. Hibernate의 flush 순서(엔티티 INSERT → UPDATE → 컬렉션 원소 DELETE/UPDATE/INSERT → 엔티티 DELETE, §12에서 확인한 RefreshToken과 동일 계열)상 **신규 INSERT가 orphanRemoval DELETE보다 먼저 실행**되어 아직 지워지지 않은 기존 행과 UNIQUE 제약이 충돌한다. 컬렉션 필드를 `clear()`로 비운 뒤 `saveAndFlush()`로 DELETE를 물리적으로 확정하고, 그 다음에 새 원소를 추가하는 **명시적 2단계(detach→flush→attach)** 로 풀어야 한다(`Artwork.detachImages()`/`attachImages()`, `ArtworkServiceImpl.replaceImages()` 참고). `@OrderColumn`으로 순서를 Hibernate에 위임하는 방식은 이 문제를 해결해 주지 않으므로(내부적으로 여전히 같은 flush 순서를 탄다), 순서가 있는 자식 컬렉션을 "부분 교체가 아니라 통째로 교체"하는 API가 있다면 이 패턴을 먼저 의심할 것.
- **공개 API record를 내부 도메인 엔티티 필드 타입으로 그대로 쓰던 코드는 JPA 전환 시 record 임베더블 미지원에 두 번 걸린다**: `WorkDuration`(공개 `com.atcrew.artwork` 패키지의 4필드 record)처럼 Mongo 시절엔 "아무 POJO/record나 중첩 문서로 저장 가능"이라 API 계약 record를 내부 필드 타입으로 재사용해도 문제가 없었지만, JPA는 record `@Embeddable`을 지원하지 않는다(§3.2 TermsAgreement와 동일 제약). API 계약(레코드 자체)은 바꿀 수 없으므로, `@Embeddable` 클래스로 감싸는 대신 **엔티티에 원시 필드 여러 개로 펼쳐 저장하고 getter/setter에서 record를 조립·분해**하는 편이 더 단순하다(`Artwork.workDurationMonths/Days/Hours/Minutes` + `getWorkDuration()`/`setWorkDuration()`). V1에 이미 이 값객체를 위한 단일 컬럼(`work_duration VARCHAR(30)`)이 잘못 설계돼 있었던 경우, 새 마이그레이션(V4)으로 여러 컬럼으로 교체했다 — 값객체 필드 수만큼 컬럼을 미리 스키마에 반영해 두는 편이 다음엔 이 재작업을 피할 수 있다.
- **MariaDB `TEXT` 컬럼과 매핑되려면 Hibernate에 `@Lob`이 아니라 `@Column(length = 65535)`을 줘야 한다**: `@Lob` String 필드는 MySQL/MariaDB 방언에서 `longtext`로 매핑되는데, V1 스키마의 `description`은 이미 `TEXT`(최대 65,535바이트)로 고정돼 있어 `@Lob`을 쓰면 `ddl-auto: validate`가 타입 불일치로 실패한다. Hibernate의 `VARCHAR` 계열 capacity-dependent DDL 타입 매핑은 길이가 방언의 `maxVarcharLength`(utf8mb4 기준 약 16,383)를 넘고 65,535 이하이면 자동으로 `text`를 선택하므로, `@Lob` 없이 `@Column(length = 65535)`만 지정하면 된다.
- **JSON 컬럼은 `@JdbcTypeCode(SqlTypes.JSON)`만으로 충분하다**: Hibernate 7.2(Spring Boot 4.0.6 번들 버전)는 `List<String>` 같은 기본 컬렉션 타입에 대해 Jackson 기반 JSON 매핑을 내장 지원한다. 별도 `AttributeConverter`나 커스텀 `UserType` 없이 `@JdbcTypeCode(SqlTypes.JSON)`만 붙이면 MariaDB `JSON` 컬럼(내부적으로 `LONGTEXT` alias)과 왕복이 정확히 검증됐다(`artwork_materials.targets/attachment_keys/links`, `artworks.video_links`, `orphaned_image_keys.keys_json`).
