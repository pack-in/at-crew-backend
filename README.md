...
<picture>
  ...
  <img alt="AT-CREW 시스템 아키텍처 — 요청 경로, 비동기 이미지 파이프라인, 관측·알람 경로" src="docs/assets/architecture-light.svg" width="100%">
</picture>
...
각 항목은 **무엇을 왜 골랐는지**와 함께 **그 대가로 무엇을 버렸는지**를 같이 적습니다.
근거가 되는 대안 검토는 링크한 설계 문서에 남아 있습니다.
...
> **버린 것** — 모듈 간 코드 재사용의 편의. 다른 모듈의 `internal`이 필요해지면 우회가 아니라
> 공개 인터페이스 설계나 모듈 재배치로 풀어야 합니다.
...
> **버린 것** — 당장의 속도. 운영 중이던 artwork를 리팩터링해야 해서 recruit에 복제하는 쪽이 더 쌌지만,
> Worker 프로토콜이 바뀔 때 N개 모듈을 각각 고치는 비용을 선불로 치렀습니다.
...
> **버린 것** — 인덱스 효율. 36바이트 키라 8바이트 PK보다 인덱스가 크고 조인 비용이 높습니다.
> 병목이 실측되면 `BINARY(16)` 전환은 컬럼 타입 변경 + Converter로 국소 처리할 수 있게 열어뒀습니다.
...
> **버린 것** — 단일 저장소의 단순함. 색인 지연과 재색인 경로를 떠안는 대신, 필터 조합이 늘어날 때마다
> SQL을 다시 짜는 비용에서 벗어났습니다.

### 시간대는 UTC로 저장하고 표시에서만 변환

일본·중국·영미권 확장을 전제로 저장·연산은 전부 `Instant`(UTC)이고, 변환은 표시 계층에서 회원
시간대 기준으로만 일어납니다. 컨테이너·JVM·로그 타임스탬프까지 UTC로 못 박았습니다.
[global-timezone-strategy](docs/design/global-timezone-strategy.md)

> **버린 것** — 로그와 DB를 KST로 바로 읽던 편의. 운영 중 눈으로 읽을 때 매번 변환이 필요합니다.
...
> **버린 것** — SaaS 무료 한도라는 상한과 벤더 종속. 자체 호스팅의 무제한 보존 기간을 포기했습니다.
...
<img alt="AT-CREW API 문서 — Swagger UI에 렌더링된 18개 태그" src="docs/assets/swagger-ui.png" width="760">
